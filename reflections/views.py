...
from .forms import PromptsForm, ReflectionForm, FeedbackForm
...
def viewReflection(request, reflection_id):
    ...
    if request.method == "POST":
        ...
        allInputs = request.POST.copy()
        feedbackForm = FeedbackForm(allInputs)
        if feedbackForm.is_valid():
            feedbackSave = feedbackForm.save(commit=False)
            feedbackSave.created_by = request.user
            feedbackSave.reflection = reflection
            feedbackSave.save()
    ...
    return render(request, 'reflections/view-reflection.html', {'LEARNING_CHOICES': LEARNING_CHOICES, 'EXPERIENCE_CHOICES': EXPERIENCE_CHOICES, 'REFLECTION_CHOICES': REFLECTION_CHOICES, 'reflection': reflection, 'details': details, 'discussionForm': feedbackForm})
