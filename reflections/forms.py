...
class PromptsForm(ModelForm):
    ...
    has_experience = forms.ChoiceField(widget=forms.RadioSelect(attrs={'class':'form-check-input experience_check'}), choices=CHOICES, initial='1')
...
class ReflectionForm(ModelForm):
    ...
    tags = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control keywords', 'data-role':"tagsinput"}))
...
class FeedbackForm(ModelForm):
    reflection_feedback = forms.ChoiceField(widget=forms.RadioSelect(attrs={'class':'form-check-input'}), choices=REFLECTION_CHOICES)
    experience_feedback = forms.ChoiceField(widget=forms.RadioSelect(attrs={'class':'form-check-input'}), choices=EXPERIENCE_CHOICES)
    learning_feedback = forms.ChoiceField(widget=forms.RadioSelect(attrs={'class':'form-check-input'}), choices=LEARNING_CHOICES)
    feedback = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control'}))
